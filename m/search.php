		...
		<?php
			if (($_GET['keywords'] && $_GET['topic']) != NULL) {
				...
				if (isset($_GET['keywords'])) {
					...
					if ( $gender_user_db == '1') {
					$query = $con,"SELECT  id,username,first_name,profile_pic,cover_pic,city,hometown,company,school,gender,verify_id FROM users where (username like '%$search_value%' OR first_name like '%$search_value%') AND (gender !='2' AND activated !='0')";
					}else if ( $gender_user_db == '2') {
					$query = $con,"SELECT  id,username,first_name,profile_pic,cover_pic,city,hometown,company,school,gender,verify_id FROM users where (username like '%$search_value%' OR first_name like '%$search_value%') AND (activated !='0')";
					}
				...
				if ($count == 0){
					...
					echo '<div style="padding: 10px;"><div class="search_banner">Result for: 
						...
						while ($row=mysqli_fetch_array($query)) {
							...
							<div style=' padding: 8px 0; min-height: 80px;'> ";
								...
								echo "<div style='margin-left: 74px; line-height: 18px; font-size: 14px;'><b><a href='profile.php?u=".$username."' style='text-decoration: none; font-size: 14px; color: #ef8d02;' title=\"Go to ".$first_name."'s Profile\" class='posted_by'>".$first_name."</a></b> <br>";
								if ($user == $username) {
									echo '<div class="">
								...
								if ($school == "") {
										...
										echo 'Work at <span style="color: #ef8d02;">'.$company.'</span><br>' ;
									}
									}else {
										echo 'Study at <span style="color: #ef8d02;">'.$school.'</span><br>' ;
									}if ($city == "") {
											if ($hometown == "") {
											//nothing
										}else {
											echo 'From <span style="color: #ef8d02;">'.$hometown.'</span>' ;
										}
									}else {
										echo 'Lives in <span style="color: #ef8d02;">'.$city.'</span><br>' ;
									}
								echo "</div>
					...
					$search_for = $search_value;
					$query = $con,"SELECT  id,body,date_added,added_by,photos,user_posted_to,discription FROM posts where  (body like '%$search_value%')  AND (note_privacy !='onlyme') ORDER BY id DESC";
					$query = mysqli_query($query) or die ("could not count");
					$count = mysqli_num_rows($query);
				if ($count == 0){
					echo '<div class="search_banner">No match found!
					...
					$search_for = $search_value;
					$query = $con,"SELECT  id,body,date_added,added_by,photos FROM ShareTime where body like '%$search_value%' ORDER BY id DESC";
					$query = mysqli_query($query) or die ("could not count");
					$count = mysqli_num_rows($query);
				if ($count == 0){
					echo '<div class="search_banner">No match found!
