...
if (isset($_REQUEST['pid'])) {
	...
	$db_username = $get_file_name['added_by'];
	$u_posted_to= $get_file_name['user_posted_to'];
	$shareShareTimeid = $get_file_name['ShareTime_post'];
	if($shareShareTimeid != 0) {
		$get_sharepost = mysqli_query($con,"SELECT  * FROM posts WHERE id='$shareShareTimeid'");
		$post_info = mysqli_fetch_assoc($get_sharepost );
		$sp_id = $post_info['id'];
		$sp_body = $post_info['body'];
	}
...
if(isset($_POST['share'])) {
	...
	$sqlCommand = $con,"INSERT INTO posts(body,date_added,added_by,user_posted_to,share_post) VALUES('$post', '$date_added', '$added_by', '$user_posted_to', '$id')";
	$query = mysqli_query($sqlCommand) or die (mysqli_error());
	header("Location: newsfeed.php");
}else if(isset($_POST['ShareTime'])) {
	$user_posted_to = $user;
	$date_added = date("Y-m-d");
	$added_by = $user;
	
	$sqlCommand = $con,"INSERT INTO posts(body,date_added,added_by,user_posted_to,ShareTime_post) VALUES('$post', '$date_added', '$added_by', '$user_posted_to', '$id')";
	$query = mysqli_query($sqlCommand) or die (mysqli_error());
	header("Location: index.php");
}else {
			...
			echo '
				...
				<div style="width: 100%;">
					...
					if (isset($_REQUEST['pid'])) {
						...
						echo'<input name="ShareTime" value="ShareTime" type="submit" class="confirmSubmit"></input>';
					}
					echo'</div>
					<div class="cc">
