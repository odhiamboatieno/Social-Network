						...
						if (mysqli_num_rows($check)===1) {
							...
							if ($pro_nums != 0) {
								...
								if ($pro_changed_dbs != $profile_pic_dbs) {
									$profile_pics = "http://www.ShareTime.com/img/default_propic.png";
								}else {
									$profile_pics = 'http://www.ShareTime.com/userdata/profile_pics/'.$profile_pic_dbs.'';
								}
							}else {
								$profile_pics = "http://www.ShareTime.com/img/default_propic.png";
							}
							...
							if ($username != $user) {
								...
								$sql=$con,"SELECT  DATE_FORMAT(chatOnlineTime,'%Y-%m-%e %H:%i:%s') as dd,DATE_FORMAT(now(),'%Y-%m-%e %H:%i:%s') as now from users where username='$username'";
								...
								while ($msg = mysqli_fetch_assoc($get_message)) {
									...
									while ($row = mysqli_fetch_assoc($emoticonQuery)) {
										...
										<div style='display: flex; padding: 8px 0; border-top: 1px solid #ddd;''>
											<div style='width: 100%;'>
												<b><a href='profile.php?u=$user_by' style='text-decoration: none; font-size: 14px; color: #ef8d02;' title=\"Go to $posted_by's Profile\" class='posted_by'>$posted_by</a></b>
