	...
	if ($shareby_gender_value == 2) {
		if (($ShareTime_to == $user) || ($user_gender_value == 2)) {
			...
			<form action='tommycomments.php?id=".$getid."' method='POST' name='ShareTimeComment".$getid."'>
				<input style='padding: 10px 3px; min-width: 60px; width: 73%; margin: 0 0 5px 0; border: 1px solid #ef8d02;' name='ShareTime_body' placeholder= 'Leave your comment here!' autofocus>
		...
		$get_msg_num = mysqli_query($con,"SELECT  * FROM pvt_messages WHERE user_from='$ShareTime_to' AND user_to='$user' LIMIT 2");
			...
			if ($female_msg >=1 ) {
				echo "
					<div>
					<form action='tommycomments.php?id=".$getid."' method='POST' name='ShareTimeComment".$getid."'>
						<input style='padding: 10px 3px; min-width: 60px; width: 73%; margin: 0 0 5px 0; resize: none; border: 1px solid #ef8d02;' name='ShareTime_body' placeholder= ''>
						<input type='submit' name='ShareTimeComment".$getid."' class='commentSubmit' value='Comment'>
					</form>
					</div>
				";
