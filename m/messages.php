...
<div class="pro_body">
	...
	<div style="width: 100%;">
		...
		<h2 style='font-size: 20px';><a href="messages.php" style="text-decoration: none; color: #06228e;">Message</a>  |  <a href="buddylist.php" style="text-decoration: none; color: #828182;" >Chat(<?php echo $buddyList; ?>)</a></h2><br />
			<?php 
			...
			while ($get_msg = mysqli_fetch_assoc($grab_messages)) {
				...
				*/
				...
				//check for userfrom propic delete
					...
					if ($pro_num == 0) {
						$profile_picuser_from = "http://www.ShareTime.com/img/default_propic.png";
					}else {
						$pro_changed_db = $get_pro_changed['photos'];
					if ($pro_changed_db != $profile_picuser_from_db) {
						$profile_picuser_from = "http://www.ShareTime.com/img/default_propic.png";
					}else {
						$profile_picuser_from = "http://www.ShareTime.com/userdata/profile_pics/".$profile_picuser_from_db;
					}
					}
				//check for userto propic delete
					...
					$pro_num = mysqli_num_rows($pro_changed);
					if ($pro_num == 0) {
						$profile_picuser_to = "http://www.ShareTime.com/img/default_propic.png";
					}else {
						$pro_changed_db = $get_pro_changed['photos'];
					if ($pro_changed_db != $profile_picuser_to_db) {
						$profile_picuser_to = "http://www.ShareTime.com/img/default_propic.png";
					}else {
						$profile_picuser_to = "http://www.ShareTime.com/userdata/profile_pics/".$profile_picuser_to_db;
					}
					}
				...
				if (strlen($msg_body) > 300) {
					...
					if ($user_from == $user) {
						...
						echo "<div><b style='font-size: 14px; font-weight: bold; color: #ef8d02;'>$msg_to_fname</b> <br>
						...
						echo "<div><b style='font-size: 14px; font-weight: bold; color: #ef8d02;'>$msg_by_fname</b> <br>
