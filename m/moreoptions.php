...
if (isset($_REQUEST['pid'])) {
	...
	$shareShareTimeid = $post_info['ShareTime_post'];
...
if (isset($_POST['continue']) and ! empty($_POST['continue'])) {
    if (isset($_POST['action_key'])) {
		...
		//delete all like
		$resultdl = mysqli_query("DELETE FROM post_likes WHERE post_id='$id'");
		...
		$resultdlt = mysqli_query("DELETE FROM post_comments WHERE post_id='$spostid'");
		$resultdl = mysqli_query("DELETE FROM post_likes WHERE post_id='$spostid'");
		}
		//delete all comments
		$resultd2 = mysqli_query("DELETE FROM post_comments WHERE post_id='$id'");
		//delete share post
		$result1 = mysqli_query("DELETE FROM posts WHERE share_post='$id'");
		//delete post
		$result3 = mysqli_query("DELETE FROM posts WHERE id='$id'");
        ...
        }else if($radio_input == "ShareTime_POST") {
        	$_SESSION['ShareTimeid'] = $id;
		
		if($shareShareTimeid != 0) {
			$_SESSION['ShareTimeid'] = $sp_id;
			header("location: sharepost.php?did=$shareShareTimeid");
		}else {
									...
									<div class="v">
										<table class="c">
											<tbody>
												<tr>
													<td class="radio">
														<input name="action_key" value="ShareTime_POST" type="radio"></input>
													</td>
													<td>
														<div class="v">
															<div>
																<span class="c_b">Add to ShareTime</span>
															</div>
															<div>
																<span class="c_g">This post show in ShareTime timeline.</span>
															</div>
														</div>
													</td>
												</tr>
