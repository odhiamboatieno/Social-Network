		...
		<?php
			if (($_GET['keywords'] && $_GET['topic']) == NULL) {
				...
				if (isset($_GET['keywords'])) {
					...
					$query = $con,"SELECT  id,username,first_name,email,gender,verify_id,sign_up_date,activated FROM users where username like '%$search_value%' OR first_name like '%$search_value%' OR email like '%$search_value%'";
				...
				if ($count == 0){
					...
					$search_for = $search_value;
					$query = $con,"SELECT  id,body,date_added,added_by,photos,user_posted_to,discription,newsfeedshow,report,note FROM posts where body like '%$search_value%' ORDER BY id DESC";
					$query = mysqli_query($query) or die ("could not count");
					$count = mysqli_num_rows($query);
				if ($count == 0){
					echo '<div class="search_banner">No match found!
					...
					$search_for = $search_value;
					$query = $con,"SELECT  id,body,date_added,added_by FROM ShareTime where body like '%$search_value%' ORDER BY id DESC";
					$query = mysqli_query($query) or die ("could not count");
					$count = mysqli_num_rows($query);
				if ($count == 0){
					echo '<div class="search_banner">No match found!
