<?php 
	...
	if (isset($_POST['login'])) {
		if (isset($_POST['admin_user']) && isset($_POST['admin_pass'])) {
			$admin_user = mysqli_real_escape_string($_POST['admin_user']);	
			$admin_pass = mysqli_real_escape_string($_POST['admin_pass']);	
			$num = 0;
			$password_login_md5 = md5($admin_pass);
			$result = mysqli_query($con,"SELECT  * FROM admin_login WHERE user_name='$admin_user' AND user_pass='$password_login_md5'");
...
<form method="POST" action="login.php" style="padding: 84px 15px;">
	<table class="admintable" width="450px" border="10" align="center" bgcolor="#ff9800" style="margin: 0 auto;">
		<tr>
