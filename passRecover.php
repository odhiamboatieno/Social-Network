		...
		if ($get_block_fetch_query == 1 ) {
			...
			$succes_echoEmail = "
				...
				<form action='passRecover.php' method='POST'>
					<span style='font-size:  margin: 13px 0 0 13px;  font-weight: 800; color:  #14088A;'>".$fullname_db."</span></br>
...
if (isset($_POST['searchRecovEmail'])) {
	...
	if ($searchRecovEmail_query_num >= 1) {
		...
		if (mysqli_query($confirmCodeQuery)) {
			...
			// send email
			$msg = "Hello ".$get_first_nm_fetch_query."
			...
			";
			//mail($recovEmail,"ShareTime Confirmation Code",$msg, "From:ShareTime <no-reply@ShareTime.com>");
			$success_email_cnfrm = "
				<form action='passRecover.php' method='POST'>
...
if (isset($_POST['searchRecovEmail2'])) {
	...
	if ($searchRecovEmail_query_num >= 1) {
		...
		if (mysqli_query($confirmCodeQuery)) {
			...
			N.B. If it's not by you, delete this mail or forget it.
			";
			mail($user_con_email,"ShareTime Confirmation Code",$msg, "From:ShareTime <thomasatieno12@gmail.com>");
			$success_email_cnfrm = "
				<form action='passRecover.php' method='POST'>
						Verification Code:</br></br><input type='text' name='confirmMailCode' class='placeholder' size='30' required autofocus></br></br>
					</hr>
			...
			if ($confirmMailCode == $get_confirmCode_fetch_query) {
				if ($newpassword == $repear_password) {
					...
					if (mysqli_query($updatePassQuery)) {
						...
						// send email
						$msg = "Successfully your ShareTime password has been changed. 
						
						If it's not by you, please follow the link.
				
						http://www.ShareTime.com/confirmationPass.php?u=".$get_uName_fetch_query."&code=".$get_confirmCode_fetch_query ."
						
						";
						//mail($get_uEmail_fetch_query ,"ShareTime Password Changed",$msg, "From:ShareTime <no-reply@ShareTime.com>");
					}
				}else {
		...
		<div class="pass_body2">
				<?php
					if (isset($success_msg)) {
						...
						echo '
							<form action="passRecover.php" method="POST">
								<p>Find your ShareTime account!</p></br>
