				...
				if ($num1>0) {
					...
					$success_message = '
						...
						<form action="signin.php" method="POST">
							...
							<input class="submRecov" type="submit" name="submconfrmCode" id="senddata" value="Continue ShareTime">
						</form>
						</div>
						';
					...
					if ($num1>=1) {
						$success_message = '
						<h2><font face="bookman">Unfortunately !!!</font></h2>
							<div class="maincontent_text" style="text-align: center;">
							<font face="bookman">This account has been blocked. Please contact system admin for assistance<br>
							</font></div>';
					}else {
						$success_message = '
...
if(isset($_POST["name2check"]) && $_POST["name2check"] != ""){
    ...
    if ($uname_check < 1) {
	    echo '<p style="color: #ef8d02; font-size: 13px; font-weight: 600; text-align: center; margin: 3px 0;">Success! Remember username for login</p>';
	    exit();
    } else {
...
$_POST['username'] = preg_replace('/\s+/','',$_POST['username']);
	try {
		...
		$e_check = mysqil_query($con,"SELECT  email FROM users WHERE email='$u_email'");
			...
			if ($check == 0 ) {
				if ($email_check == 0) {
					if (strlen($_POST['password']) >4 ) {
						...
						";
						//if (@mail($_POST['email'],"ShareTime Activation Code",$msg, "From:ShareTime <no-reply@ShareTime.com>")) {
...
//getting ShareTime post photo
$getposts = mysqli_query($con,"SELECT  * FROM ShareTime WHERE photos != '' ORDER BY  RAND() ");
$row = mysqli_fetch_assoc($getposts);
$photos_db = $row['photos'];
$photosrow = "./userdata/tommy_pics/".$photos_db;
...
<html>
	<head>
	<title>ShareTime For you</title>
	<meta charset="uft-8">
	<link rel="icon" href="./img/title.png" type="image/x-icon">
	<link rel="image_src" href="http://www.ShareTime.com/userdata/tommy_pics/nur/1453542261.jpg" /><!--formatted-->
	<link rel="stylesheet" type="text/css" href="./css/style.css">
	<meta name="description" content="Assalamu Alaikum. Join ShareTime. Connect with your family. ShareTime to muslim brother. Get update over the world." /><!--formatted-->
		...
		<div class="main">
			<div class="headerLogin">
				<div class="login_menubar clearfix">
					<div class="menu_logo">
						<h1>
							<a title="Go to ShareTime Home" href="index.php">
								<b>ShareTime</b>
							</a>
						</h1>
					</div>
							...
							<div class="maincontent">
							   <?php
								if (isset($success_message)) {
									...
									echo '
									<center><href="https://www.neo.com"><img src="img/login.png" width="150" height="150"></center>
									   <h2><font face="bookman">Welcome to ShareTime</font></h2>
										<div class="maincontent_text">
										<font face="bookman">Here yo will : <br>
											<li>Connect with your family.</li>
											<li>Connect with  friends.</li>
											<li>Get updates all  over the world.</li>
										</font>
										</div>
												...
												<td>
													<th>
														<div style="float: left;padding: 13px 13px 0 13px;font-size: 16px;font-weight: bold;">
															<input type="radio" name="gender" value="Male" requred checked/><span>Male</span>
														</div>
													</th>
													<th>
														<div style="float: left;padding: 13px 13px 0 13px;font-size: 16px;font-weight: bold;">
