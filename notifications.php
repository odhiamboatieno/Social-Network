	...
	<?php include ( "./inc/header.inc.php" ); ?>
		<div style="width: 900px; margin: 52px auto;">
				...
				<div class="homeLeftSideContent">
					<div class="home_cov" style= "background: url(<?php echo $cover_pic; ?>) repeat center center;">
						...
						<div class="homenavemanu">
							<div >
								<div ><a href="index.php">ShareTime</a></div>
			...
			<div style="float: right;">
				 <div class="notificationbox">
				 	<a href="notifications.php"><h2 style="float: left; color: #ef8d02;">ShareTime Notification&nbsp;|</h2></a><a href="notification.php"><h2 style="float: left;color: #848684;">&nbsp;Post Notification</h2></a>
				 			...
				 			<li style='line-height: 8px;'>
				 				<?php 
									...
									$count_notipost = mysqli_num_rows($get_notipost);
				 					//getting ShareTime comment
									$get_notishare = mysqli_query($con,"SELECT  * FROM ShareTime_comments WHERE ShareTime_to='$user' AND ShareTime_by != '$user' ORDER BY id DESC LIMIT 50");
									$count_notishare = mysqli_num_rows($get_notishare);
									$count = $count_notishare + $get_notishare;
									if ($count != 0) {
										//getting ShareTime noti
										while ($noti_share = mysqli_fetch_assoc($get_notishare)) {
											$ShareTime_id = $noti_share['ShareTime_id'];
											$ShareTime_body = $noti_share['ShareTime_body'];
											$time = $noti_share['time'];
											$user_by = $noti_share['ShareTime_by'];
											$user_to = $noti_share['ShareTime_to'];
		...
		if ($pro_changed_db != $profile_pic_db) {
			...
			$profile_pic = "userdata/profile_pics/".$profile_pic_db;
											...
											$get_descrip = mysqli_query($con,"SELECT  * FROM ShareTime WHERE id='$ShareTime_id'");
											$get_description = mysqli_fetch_assoc($get_descrip);
											$discription = $get_description['photos'];
											if ($discription == "") {
												$comment_disp = "commented on your ShareTime";
												$posted_pic = "";
											}else {
												$comment_disp = "commented on your ShareTime photo";
												$posted_pic = "userdata/tommy_pics/".$discription;
											}
											if ($discription == "") {
												$comment_disp = "commented on your ShareTime";
											}else {
											...
											echo "<div style='margin: 10px; padding: 7px 0px;'>";
												...
												echo "<div style='margin: 0 0 0 50px; line-height: 1.4;'>";
													echo "<b><a href='profile.php?u=$user_by' style='text-decoration: none; font-size: 14px; color: #ef8d02;' title=\"Go to $posted_by's Profile\" class='posted_by'>$posted_by</a></b><span style='font-size: 15px; margin-left: 10px;'>".$comment_disp."</span>
													"; 
														 if ($discription == "") {
														 	//
														 }else {
														 	echo "<a href='viewtommy.php?did=".$ShareTime_id."'><img src='$posted_pic' style= 'float: right;' height='55' width='65' /></a>";
														 }
													echo "<br>";
														echo "<span style='line-height: 1.5; color: #444;' >".nl2br($ShareTime_body)."</span><br>
