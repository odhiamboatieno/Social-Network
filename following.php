...
<body>
	...
	echo '<li style="float: right;">
					...
					<div >
						...
						<li><a href="friends.php?u='.$username.'" style="background-color: #cdcdcd; color: #06228e">Friend</a></li>
	...
	if (($msg_count >=1 ) || ($username == $user)) {
			...
			$count_frnd_num = 0;
				//count following
				$queryfollowing = $con,"SELECT  * FROM follow WHERE user_from='$username' ORDER BY id DESC";
				$queryfollowing= mysqli_query($queryfollowing) or die ("could not count");
				$countfollowing = mysqli_num_rows($queryfollowing);
				//count followers
				$queryfollowers = $con,"SELECT  * FROM follow WHERE user_to='$username' ORDER BY id DESC";
				$queryfollowers = mysqli_query($queryfollowers ) or die ("could not count");
				$countfollowers = mysqli_num_rows($queryfollowers );
				//getting all friend
				$queryfollowing2 = $con,"SELECT  * FROM follow WHERE user_from='$username' ORDER BY id DESC";
				...
				echo '<div>
					     <table class="friendsmenu">
							<tr>
								...
								<th style="background-color: #01093a;"><a href="following.php?u='.$username.'" ><h1>Following </br>'.$countfollowing.'</h1></a></th>
		...
		$pro_num = mysqli_num_rows($pro_changed);
		if ($pro_num == 0) {
			$cover_pic_picuser_to = "img/sharetime_defaultcover.png";
		}else {
			$pro_changed_db = $get_pro_changed['photos'];
