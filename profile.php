	...
	if ($num == 1) {
			...
			echo '<li style="float: right;">
							...
							<div >
								...
								<li><a href="profile.php?u='.$username.'" style="background-color: #cdcdcd; color: #06228e">Post</a></li>
								</ul>
								</nav>
							</div>
			...
			if (($msg_count >=1 ) || ($username == $user)){
				...
				//post update
					...
					if ($post != "") {
						...
						$sqlCommand = $con,"INSERT INTO posts(body,date_added,added_by,user_posted_to,newsfeedshow ) VALUES('$post', '$date_added','$added_by', '$user_posted_to', '$newsfeedshow')";
						$query = mysqli_query($sqlCommand) or die (mysqli_error());
					}

				//for getting post

				$getposts = mysqli_query($con,"SELECT  * FROM posts WHERE user_posted_to ='$username' AND ShareTime_give='0' AND note='0' AND report='0' ORDER BY id DESC LIMIT 9") or die(mysqli_error());
