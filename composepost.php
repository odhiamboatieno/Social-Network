...
//check for userfrom propic delete
 ...
 <?php 
	...
	$ShareTime = mysqli_real_escape_string($post);
	...
	if ($pic != "") {
		...
		if (((@$_FILES['uploadFile']['type']=='image/jpeg') || (@$_FILES['uploadFile']['type']=='image/png') || (@$_FILES['uploadFile']['type']=='image/gif')) && (@$_FILES['uploadFile']['size'] < 200000)) {
			...
			if (file_exists($photoLocat."/userdata/profile_pics/$chare/".@$_FILES["uploadFile"]["name"])) {
				...
				$sqlCommand = $con,"INSERT INTO posts(body,date_added,added_by,user_posted_to,photos) VALUES('$post', '$date_added','$added_by', '$user_posted_to', '$photos')";
				...
				$date_added = date("Y-m-d");
				$added_by = $user;
				$sqlCommand = $con,"INSERT INTO posts(body,date_added,added_by,user_posted_to) VALUES('$post', '$date_added', '$added_by', '$user_posted_to')";
				$query = mysqli_query($sqlCommand) or die (mysqli_error());
				header("Location: newsfeed.php");
			}
			else {
	...
	<div class="pro_body">
		<div class="pro_header">
			<nav class="pro_hdr_menu">
				<ul>
					<li><a href="http://sharetime.epizy.com/index.php">ShareTime</a></li>
					<li><a href="http://sharetime.epizy.com/newsfeed.php">Newsfeed</a></li>
					<li><a href="http://sharetime.epizy.com/profile.php?u=<?php echo "$user"; ?>">Profile</a></li>
					<li><a href="http://sharetime.epizy.com/messages.php">Message</a></li>
					<li><a href="http://sharetime.epizy.com/followRequest.php">Friends</a></li>
					<li><a href="http://sharetime.epizy.com/notification.php">Notification</a></li>
					<li><a href="http://sharetime.epizy.com/search.php">Search</a></li>
				</ul>
			</nav>
		</div>
		<?php echo $error; ?>
