...
$ShareTimepostid = $row['ShareTime_post'];
...
if (strlen($body) > 4000) {
		$body = substr($body, 0, 2000)." <a href='viewPost.php?pid=".$id."' style='text-decoration: none; color: #ef8d02;'>... read more</a>";
	}else {
		$body = $body;
	}
...
if($sharepostid != 0) {
	...
	if($sp_added_by == $added_by) {
		if ($gender_user_db == '1') {
			$discription = "shared his <a href='viewPost.php?pid=".$sharepostid."' style='text-decoration: none; padding-left: 0px; color: #ef8d02;'>".$post_item."</a>";
		}else if ($gender_user_db == '2') {
			$discription = "shared her <a href='viewPost.php?pid=".$sharepostid."' style='text-decoration: none; color: #ef8d02;'>".$post_item."</a>";
		}
	}else {
		$discription = "shared ".$sp_add_by."'s <a href='viewPost.php?pid=".$sharepostid."' style='text-decoration: none; padding-left: 0px; color: #ef8d02;'>".$post_item."</a>";
	}
}
		...
		echo "<div style='min-height: 55px;' ><div style='float: left;'><img src='$profile_pic' style= 'border-radius: 22px'; title=\"$added_by\" height='45' width='45'  /></div>";
			...
			echo'<div class="posted_by">
				<span style="color: #9e9e9e;" >
					...
					echo'<a href="profile.php?u='.$added_by.'" style="text-decoration: none; color: #ef8d02;">'.$add_by.'</a><span> '.$discription.'</span>';
					}else {
					echo'<a href="profile.php?u='.$sp_added_by.'" style="text-decoration: none; color: #ef8d02;">'.$add_by.'</a> > <a href="profile.php?u='.$user_posted_to.'" style="text-decoration: none; color: #ef8d02;">'.$post_to_fname.'</a>';
					}
			...
			if($sharepostid != 0) {
				...
				$sp_sharebody = $post_info['ShareTime_body'];
				$sp_discription = $post_info['discription'];
				$sp_date_added = $post_info['date_added'];
				$sp_sharegive = $post_info['ShareTime_give'];
				$sp_photos_db = $post_info['photos'];
				$sp_photos = "http://www.ShareTime.com/userdata/profile_pics/".$sp_photos_db;
				if($sp_sharegive != 0) {
						$sp_body = $sp_sharebody;
					}
					
				if (strlen($sp_body) > 1500) {
						$sp_body = substr($sp_body, 0, 1500)." <a href='viewPost.php?pid=".$sp_id."' style='text-decoration: none; color: #ef8d02;'>... read more</a>";
					}else {
						$sp_body = $sp_body;
					}
					...
					<div style="padding: 10px;" >';
						...
						<div style=" padding-left: 10px; overflow: hidden; ">
							<span style="color: #9197a3;" >
								<span style="font-weight: bold;">
									<a href="profile.php?u='.$sp_added_by.'" style="text-decoration: none; color: #ef8d02;">'.$sp_add_by.'</a>
								</span>
							</span>
			...
			if ($rows_uname == 1) {
				echo "<a href='like.php?upid=".$id."' style='color: #ef8d02;' >Liked . $count_like</a>";
			}else {
